...
`go-ruby-mail/mail` reimplements Ruby's `mail` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-mail/mail` directly, with no Ruby runtime;
